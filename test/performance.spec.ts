import { getExtractedFields, RequestLike } from '../src/key/field-extractor';
...
describe('performance: getExtractedFields', () => {
  it('small request (body + header) averages < 0.5 ms per call', () => {
    ...
    const avg = measureMs(() => getExtractedFields(req, ['accountId'], ['x-request-id']), ITERATIONS);
  ...
  it('50 body fields averages < 2 ms per call', () => {
    ...
    const avg = measureMs(() => getExtractedFields(req, fields), ITERATIONS);
    expect(avg).toBeLessThan(2);
  });

  it('missing fields averages < 0.5 ms per call', () => {
    const req: RequestLike = { headers: {}, body: {}, query: {}, params: {} };
    const avg = measureMs(() => getExtractedFields(req, ['missing1'], ['x-absent'], ['q-missing'], ['p-missing']), ITERATIONS);
    expect(avg).toBeLessThan(0.5);
  });
});
